<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($id) {
            $stmt = $conn->prepare("UPDATE students SET school_id=?, name=?, email=?, phone=?, department=?, section=?, image=? WHERE id=?");
            $stmt->bind_param("sssssssi", $school_id, $name, $email, $phone, $department, $section, $image_path, $id);
        } else {
            $stmt = $conn->prepare("INSERT INTO students (school_id, name, email, phone, department, section, image) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssssss", $school_id, $name, $email, $phone, $department, $section, $image_path);
        }
        $stmt->execute();
